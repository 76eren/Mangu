...
public class Mangakakalot implements Sources {
    ...
    public void GetLinks(String manga) {
        // We want to empty this so it doesn't fucking explode.
        images.clear();
        urls.clear();
        names.clear();
        ...
        for (Element i : elems.first().children()) {
            ...
            images.add(img); // IMPLEMENTED FROM THE INTERFACE
            urls.add(link); // IMPLEMENTED FROM THE INTERTFACE
            names.add(name); // IMPLEMENTED FROM THE INTERFACE
